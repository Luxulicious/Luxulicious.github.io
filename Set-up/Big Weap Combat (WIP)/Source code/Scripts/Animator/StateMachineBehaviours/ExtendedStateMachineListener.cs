...
public class ExtendedStateMachineListener : MonoBehaviour, IExtendedStateMachineListener
{
    ...
    public bool calculateHashInPlay = false;
    ...
    void Update()
    {
        ...
        //TODO Optimize this loop with an observe/callback or custom editor script
        if (!Application.isPlaying)
            _stateCallbacks.ForEach(x => x.CalculateHash());
        else if (calculateHashInPlay && Application.isEditor)
            _stateCallbacks.ForEach(x => x.CalculateHash());
    }
    ...
    {
        //TODO Optimize the getting of the current state at runtime...
        _currentStateCallback = _stateCallbacks.FirstOrDefault(x => stateInfo.fullPathHash == x.hash);
